...
const COMPONENT_NAME = "apple-intelligence";
...
class KDEShortcutManager {
  ...
  _findCallbackByFriendlyName(name) {
    ...
    for (const slotName of this.registeredSlots) {
      friendlyToSlot[`Apple Intelligence ${slotName}`] = slotName;
      friendlyToSlot[`Apple Intelligence`] = "dictation"; // legacy compat
    }
  ...
  async registerKeybinding(electronHotkey, slotName = "dictation", callback) {
    ...
    const actionId = [COMPONENT_NAME, slotName, "Apple Intelligence", `Apple Intelligence ${slotName}`];
