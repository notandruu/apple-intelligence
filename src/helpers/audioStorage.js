  ...
  _buildFilename(transcriptionId, timestamp) {
    if (timestamp) {
      ...
      if (!isNaN(d.getTime())) {
        ...
        return `Apple Intelligence-${date}-${time}-${transcriptionId}.webm`;
      }
    }
    return `Apple Intelligence-${transcriptionId}.webm`;
  }
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (stats.mtimeMs < cutoffMs) {
            fs.unlinkSync(filePath);
            // Extract ID from "Apple Intelligence-...-{id}.webm" or legacy "{id}.webm"
