#!/bin/bash
# Post-install script for Apple Intelligence (deb/rpm)
...
CHROME_SANDBOX=$(dpkg -L apple-intelligence 2>/dev/null | grep chrome-sandbox || echo "")
if [ -z "$CHROME_SANDBOX" ]; then
  # Fallback: conventional electron-builder install path
  CHROME_SANDBOX="/opt/Apple Intelligence/chrome-sandbox"
fi
