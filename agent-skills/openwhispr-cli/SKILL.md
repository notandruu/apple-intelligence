...
description: Use this skill whenever the user wants to operate on Apple Intelligence notes, folders, transcriptions, or audio from a terminal or shell. The Apple Intelligence CLI (`apple-intelligence` binary, npm package `@apple-intelligence/cli`) talks to either the local desktop app or the cloud API and exposes every operation needed for managing notes, folders, transcriptions, audio, plus auth and config. Trigger this skill when the user mentions "apple-intelligence cli", running shell commands against Apple Intelligence, automating note workflows, cleaning up a transcription, building agent integrations against Apple Intelligence, or scripting any Apple Intelligence operation — even if they don't say "CLI" explicitly.
---

# Apple Intelligence CLI

Use this reference when running the `apple-intelligence` command-line tool. The CLI is a single binary that operates against either the local desktop app (via a loopback HTTP bridge) or the cloud REST API. The same command works against both backends.

## Install

```bash
npm install -g @apple-intelligence/cli
```

Requires Node.js 20 or later. Verify with `apple-intelligence --version`. If the user reports `command not found`, ensure their npm global bin is on `$PATH`.
...
| **remote** | `https://api.apple-intelligence.com/api/v1`               | Desktop is closed, or running on a different machine, or the user wants cloud-side semantics. |
...
3. The `backend` key in `~/.apple-intelligence/cli-config.json`
...
When the desktop app starts, it writes `{version, port, token}` to `~/.apple-intelligence/cli-bridge.json` with mode `0600`. The CLI reads it automatically. If the file is missing or stale, local is treated as unavailable.
...
```bash
apple-intelligence auth login    # prompts for the key, stores it 0600 in ~/.apple-intelligence/cli-config.json
apple-intelligence auth status   # confirm it works
apple-intelligence auth logout   # clear it
```
...
Noun-verb syntax: `apple-intelligence <noun> <verb>`. Same convention as `gh`, `kubectl`, `aws`, `stripe`.

### Notes

```bash
apple-intelligence notes list [--folder <id>] [--limit N] [--format json|table]
apple-intelligence notes get <id> [--format json|markdown]
apple-intelligence notes create --content <text> | --content-file <path>
                        [--title <t>] [--folder <id>]
apple-intelligence notes update <id> [--content <t>] [--folder <id>] [--title <t>]
apple-intelligence notes delete <id> [--dry-run] [--format json]
apple-intelligence notes search <query> [--limit N] [--format json|table]
```

### Folders

```bash
apple-intelligence folders list [--format json|table]
apple-intelligence folders create --name <name> [--sort-order <n>]
```
...
```bash
apple-intelligence transcriptions list [--limit N] [--format json|table]
apple-intelligence transcriptions get <id> [--format json|text]
apple-intelligence transcriptions delete <id> [--dry-run] [--format json]
```
...
```bash
apple-intelligence audio delete <transcription-id> [--format json]
```
...
```bash
apple-intelligence auth login [--api-key <key>]   # prompts on stdin if --api-key omitted
apple-intelligence auth status [--format json]
apple-intelligence auth logout
```

`auth status` reads the stored config and reports whether a key is configured — it does **not** make a network call. To verify the key actually works, use `apple-intelligence doctor`.

### Config

```bash
apple-intelligence config get [--format json]
apple-intelligence config set backend auto|local|remote
apple-intelligence config set api-base https://api.apple-intelligence.com
```
...
```bash
apple-intelligence doctor [--format json]
```
...
```bash
apple-intelligence --version    # or: apple-intelligence version
```
...
```bash
apple-intelligence notes list --limit 100 --format json | \
  jq -r '.[] | select(.title | contains("draft")) | .id' | \
  while read id; do
    apple-intelligence notes delete "$id"
  done
```
...
```bash
apple-intelligence notes search "quarterly budget" --format json | jq '.[].id'
```
...
| ------------------------------- | --------------------------------------- | ------------------------------------------------ |
| `~/.apple-intelligence/cli-bridge.json` | The desktop app at startup              | `{version, port, token}` for the loopback bridge |
| `~/.apple-intelligence/cli-config.json` | The CLI's `auth login` and `config set` | `{backend, apiBase, apiKey}`                     |
...
| ----------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------- |
| `Backend unreachable` (exit 2) on every command | Desktop closed and no API key                                 | Start desktop, or `apple-intelligence auth login`          |
...
| Config file readable by other users             | File created or edited outside the CLI                        | `chmod 0600 ~/.apple-intelligence/cli-config.json`         |
